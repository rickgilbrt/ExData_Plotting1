...
## pwrData3 <- cbind(pwrData2$sampleTime,data.frame(as.numeric(as.character(pwrData2[,1:6])))
## pwrData4 <- data.frame(pwrData3)
...
hist(pwrData3$Global_active_power,
        main = capwords(gsub("_"," ",names(pwrData3)[2])),
        xlab = paste(capwords(gsub("_"," ",names(pwrData3)[2])),"(kilowatts)"),
        col="red")
dev.off()
