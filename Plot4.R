...
par(mfcol = c(2,2), mar = c(5,4,4,2)) ##,oma=c(1,1,3,1))
        ## upper left chart
        plot(pwrData3$sampleTime,pwrData3$Global_active_power, 
                type = "l",
                xlab="",
                ylab=paste(capwords(gsub("_"," ",names(pwrData3)[2]))))
        ## lower left chart
        plot(pwrData3$sampleTime,pwrData3$Sub_metering_1,
                ...
                legend("topright",
                        ...
                        legend=names(pwrData3)[5:7],
                        bty = "n")
        ## upper right chart
        plot(pwrData3$sampleTime,pwrData3$Voltage, 
                type = "l",
                xlab="datetime",
                ylab=names(pwrData3)[4])
        ## lower right chart
        plot(pwrData3$sampleTime,pwrData3$Global_reactive_power, 
                type = "l",
                xlab="datetime",
                ylab=names(pwrData3)[3])        
dev.off()
